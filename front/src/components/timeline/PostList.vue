  ...
  axios
    ...
    .then((response) => {
      if (response.data.liked) {
        postStore.posts[index].Likes.splice(0, 0, response.data.data);
      } else {
        let indexOf = postStore.posts[index].Likes.findIndex((object) => {
          return object.userId == userStore.currentUser.id;
        });
        postStore.posts[index].Likes.splice(indexOf, 1);
      }
    })
    .catch((error) => {
...
function foundLike(index) {
  ...
  return postStore.posts[index].Likes.some(checkIfLiked)
}
    ...
    <article class="card" v-for="(post, index) in postStore.posts">
      <div class="card_header">
        ...
        <div class="header_edit" v-if="displayOptionBtn(post.User.id)"
          ...
          <div class="comment_edit_container" v-if="postStore.posts[index].editMode"
            v-click-outside="() => (postStore.posts[index].editMode = false)">
            <span class="edit_delete"
              @click="deletePost(post.id, index), (postStore.posts[index].editMode = !postStore.posts[index].editMode)"><i
                class="fa-solid fa-trash-can"></i> Supprimer</span>
            <span @click="postStore.posts[index].editPost = true"><i class="fa-solid fa-pen-to-square"></i>
              Editer</span>
          </div>
        </div>
